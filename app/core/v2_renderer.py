...
def render_v2_template(
    ...
    logger.info(f"manifest.scenes count: {len(manifest.scenes)}")
    
    if not manifest.scenes:
        raise ValueError("没有可渲染的场景 - 请检查数据格式是否正确（至少需要 2 个数据点）")
...
def run_v2_render(
    ...
    **kwargs,
    ...
    Args:
        ...
        brand: 品牌主题
        platform: 目标平台
        output_path: 绝对输出路径；None 时由 render_v2_template 生成默认路径
        **kwargs: 其他参数

    Returns:
        渲染结果字典，含 success / output_path / message / error
    """
    try:
