"""Minimal runnable renderer for panel_tutor.
...
from shared.config.settings import settings, get_quality_preset
from shared.content.typography import apply_matplotlib_typography
from shared.core.quality_gate import validate_render_inputs, validate_video_output
from shared.platform.presets import get_platform_preset

# Frame counts per quality tier (project-specific, not part of shared preset)
...
def describe_viz_backend() -> dict[str, str]:
    return {"project": "panel_tutor", "renderer": "matplotlib+ffmpeg"}
...
def render_video(
    ...
    dpi = contract.dpi
    total_frames = _FRAME_COUNTS.get(quality, 30)

    out_dir = settings.video.output_dir / "panel_tutor"
    frames_dir = settings.temp_dir / "panel_tutor_frames" / f"{quality}_{platform}"
    ...
    report = validate_video_output(output_path, contract)
