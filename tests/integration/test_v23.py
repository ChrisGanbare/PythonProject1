...
def check_api_endpoint(name, url, method="GET", data=None):
    """测试 API 端点（非 pytest 用例，避免以 test_ 开头被误收集）。"""
    import requests
...
def run_tests():
    ...
    # Quick video API
    check_api_endpoint("GET /api/video/v2/templates", "/api/video/v2/templates")
    check_api_endpoint("GET /api/video/v2/brands", "/api/video/v2/brands")

    # Studio 控制面
    check_api_endpoint("GET /api/studio/v1/health", "/api/studio/v1/health")
    check_api_endpoint("GET /api/registry", "/api/registry")
    ...
    success, result = check_api_endpoint(
        "POST /api/video/v2/create",
        "/api/video/v2/create",
        method="POST",
        data=create_data
    )
    ...
    if success and result:
        ...
        for i in range(10):
            time.sleep(1)
            success, status = check_api_endpoint(
                f"GET /api/video/v2/status/{job_id}",
                f"/api/video/v2/status/{job_id}"
            )
