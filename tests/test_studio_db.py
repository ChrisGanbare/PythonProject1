...
from shared.studio.services.job_lifecycle import get_job_public_dict, schedule_render_job
...
def test_run_render_updates_db(mock_run, isolated_studio_db: object) -> None:
    ...
    reg = ProjectRegistry(Path(__file__).resolve().parents[1])
    reg.discover()
    project = reg.get_project("loan_comparison")
    assert project is not None

    jid = schedule_render_job(project="loan_comparison", task="smoke_check", kwargs={})
    from shared.studio.services.job_lifecycle import run_render_job_task
