...
def test_scaffold_project_returns_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_generate(repo_root: Path, project_name: str, description: str | None = None) -> Path:
        del repo_root, description
        path = tmp_path / project_name
        created_paths.append(path)
        return path

    monkeypatch.setattr("scripts.scheduler.generate_project_scaffold", fake_generate)
