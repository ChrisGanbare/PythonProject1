      ...
      createApp({
        data() {
          return {
            ...
            /** Studio 意图会话（/api/studio/v1/sessions） */
            studioSessions: [],
        ...
        mounted() {
          ...
          // 仅在「意图会话」分区加载列表，避免默认停在「项目与任务」时仍请求 /api/studio/v1/sessions 并显示 Not Found
          if (this.sidebarPrimaryTab === 'agent') {
        ...
        methods: {
          ...
          async fetchStudioSessions() {
            ...
            this.studioSessionsSoftNotice = null;
            try {
              const res = await fetch('/api/studio/v1/sessions?limit=30');
              let data = {};
              try {
                data = await res.json();
              } catch (e) {
              ...
              if (!res.ok) {
                if (res.status === 404) {
                  ...
                  this.studioSessionsSoftNotice =
                    '未检测到会话接口（HTTP 404）。未使用「意图会话」时可忽略；需要时请用本仓库启动 dashboard 并确认已挂载 /api/studio/v1。';
                  return;
                }
          ...
          async createStudioSession() {
            ...
            try {
              const res = await fetch('/api/studio/v1/sessions', {
                method: 'POST',
          ...
          async refreshStudioSessionDetail() {
            ...
            this.studioSessionsError = null;
            try {
              const res = await fetch(`/api/studio/v1/sessions/${encodeURIComponent(this.selectedStudioSessionId)}`);
              const data = await res.json();
              if (!res.ok) throw new Error(this._apiErrorMessage(data, res.statusText));
          ...
          async updateSessionTitle(sessionId, newTitle) {
            try {
              const res = await fetch(`/api/studio/v1/sessions/${encodeURIComponent(sessionId)}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
          ...
          async cleanupStaleSessions() {
            ...
            for (const s of stale) {
              try {
                // 1. 标记已完成
                await fetch(`/api/studio/v1/sessions/${encodeURIComponent(s.id)}/complete`, { method: 'POST' });
                // 2. 删除
                await fetch(`/api/studio/v1/sessions/${encodeURIComponent(s.id)}`, { method: 'DELETE' });
          ...
          async runAgentJob() {
            ...
            try {
              const runBody = { template: payload };
              if (this.agentLinkedSessionId) {
                runBody.session_id = this.agentLinkedSessionId;
              }
              const res = await fetch('/api/studio/v1/jobs', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
              ...
              try {
                ...
                if (job.status === 'success') {
                  ...
                  if (this.agentLinkedSessionId) {
                    ...
                    // 标记会话为已完成并刷新列表
                    try {
                      await fetch(`/api/studio/v1/sessions/${encodeURIComponent(this.agentLinkedSessionId)}/complete`, {
                        method: 'POST'
                      });
